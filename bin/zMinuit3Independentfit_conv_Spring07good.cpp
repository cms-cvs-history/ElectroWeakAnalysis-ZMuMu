...
double BW (double *x, double * p) {
  ...
  double Gline = p[2]/s;
  return ( (1-fabs(p[3]))*Zline + Gline + p[3] * Int );// o 1-p[3]???
}
...
double BWZmt (double *x, double * p) {
  ...
  return 2 * p[0] * (.945)* BWZ *2* (p[5]* p[5] * p[6]* (1.0-p[6])) +  p[13]/(N1-N2) * exp( -l * x[0] )  * (1 + x[0] * ( a + x[0] * b));
...
double BWZms (double *x, double * p) {
  ...
  double N2 = exp (- l * fitXMin) / (l* l * l) * (-l * (l * fitXMin * a + a + l) - b * (l * l * fitXMin * fitXMin + 2 * l  * fitXMin + 2 ));
  */
  return   4 * p[0] * ( .639) * BWZ *2* (p[6]* p[6] * p[5]* (1.0-p[5])) 
...
void fcn( int &, double *, double & f, double * par, int ) {
  ...
  if ( firstTime ) {
    ...
    //Zmm senza back
 TFile * ZToLL_file1 = new TFile("test/EffTrackEffStandAloneHisto/ZMM_ZMT_ZMSA_HistoIso2_Spring07.root","read");
...
void zMinuit3Independentfit_conv_Spring07() {
  ...
  minuit.mnparm( 0, names[0].c_str(),80000  ,1000 , 1000, 100000000, ierflg ); 
  
  //  minuit.FixParameter( 0 );
  minuit.mnparm( 1, names[1].c_str(), 91.3, .1, 70., 110, ierflg );
  // minuit.FixParameter( 1 );
  minuit.mnparm( 2, names[2].c_str(), 2.5, 1, 2, 10, ierflg ); 
 ...
 minuit.mnparm( 12, names[12].c_str(), 0., .1, -1, 1, ierflg ); 
  minuit.mnparm( 13, names[13].c_str(), 4000, 100, 0, 100000, ierflg ); 
  minuit.mnparm( 14, names[14].c_str(), 0.001,  .001, -0, 100, ierflg ); 
  minuit.mnparm( 15, names[15].c_str(), 1000, 1, -10000, 10000, ierflg ); 
  minuit.mnparm( 16, names[16].c_str(), 1, 1, -1000, 1000, ierflg ); 

  minuit.mnparm( 17, names[17].c_str(), 30, .1, 20, 60, ierflg ); 
  // minuit.FixParameter(  );
 
 minuit.mnparm( 18, names[18].c_str(), 0.01,  0.001,-0.1, 0.1, ierflg ); 
...
zToSingleStandAloneMu->GetXaxis()->SetRangeUser(fitXMin-20, fitXMax+20);
 ...
 double p0, dp0;
   ...
   double Norm  = (p0 * 0.946 * p5 * p5* p6* p6 + p17)/ IntHisto;
...
int main() {
  //  gROOT->Reset();
  zMinuit3Independentfit_conv_Spring07();
  return 0;
}
