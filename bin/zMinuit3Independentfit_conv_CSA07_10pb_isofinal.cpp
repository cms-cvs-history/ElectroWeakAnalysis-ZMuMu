...
#include <cmath>
using namespace std;
static const int fitXMin = 80, fitXMax = 104;
...
double BW (double *x, double * p) {
  ...
  double Gline = p[2]/s;
  return ( (1-(p[3]))*Zline + Gline + p[3] * Int );// o 1-p[3]???
}
...
double BWZmt (double *x, double * p) {
  ...
  return BinWidthZmt  * p[0] * BWZ * 2 * (p[5]* p[5] * p[6]* (1.0-p[6])) +  p[13]/(N1-N2) * exp( -l * x[0] )  * (1 + x[0] * ( a + x[0] * b));
