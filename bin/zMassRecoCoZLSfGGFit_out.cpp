...
int main() { 
  ...
  ChiSquared chi2(f, zMass, 70, 120);
  int fullBins = chi2.degreesOfFreedom();
  cout << "N. deg. of freedom: " << fullBins << endl;
  fit::RootMinuit<ChiSquared> minuit(10, chi2, true);
  minuit.setParameter(0, yield1, 10, 100, 1000000);
  minuit.setParameter(1, yield2, 10, 100, 1000000);
  ...
  zMass->Draw("e");
  fun.Draw("same");
  canvas.SaveAs("ZMassRecoCoZLSfGGHistoFunFit_out.ps");
  return 0;
}
