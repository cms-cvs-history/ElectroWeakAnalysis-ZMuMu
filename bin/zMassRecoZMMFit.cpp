...
int main() { 
  ...
  function::Parameter eff_track("Track efficiency", 1.);
  function::Parameter eff_standalone("Standalone efficiency", 1.);
  ...
  function::Parameter deff_track("Track efficiency Error", 0);
  function::Parameter deff_standalone("Standalone efficiency Error", 0);
  cout << ">>> set pars: " << endl;
  ...
  cout << eff_track << " ; " << deff_track << endl; 
  cout << eff_standalone << " ; " << deff_standalone << endl; 
  function::Constant c(yield);
  function::ZMuMuFunction zmm(mass, gamma, f_gamma, f_int, 
			      mean, sigma, yield, 
			      eff_track, eff_standalone);
  typedef function::Product<function::Constant, function::ZMuMuFunction> FitFunction;
  FitFunction f = c * zmm;
  ...
  minuit.setParameter(5, eff_track, 0.0001, 0, 1.);
  minuit.setParameter(6, eff_standalone, 0.0001, 0, 1.);
  minuit.setParameter(7, mean, 0.001, -0.5, 0.5);
  minuit.fixParameter(7);
  minuit.setParameter(8, sigma, 0.1, -5., 5.);
  ...
  cout << f_int << " ; " << df_int << endl;
  deff_track = minuit.getParameterError(5);
  cout << eff_track << " ; " << deff_track << endl;
  deff_standalone = minuit.getParameterError(6);
  cout << eff_standalone << " ; " << deff_standalone << endl;
  dmean = minuit.getParameterError(7);
  cout << mean << " ; " << dmean << endl;
  dsigma = minuit.getParameterError(8);
  cout << sigma << " ; " << dsigma << endl;
  ...
  pars.push_back(eff_track.ptr());
  pars.push_back(eff_standalone.ptr());
  ...
  zMass->Draw("e");
  fun.Draw("same");
  canvas.SaveAs("ZMassRecoZMMHistoFunFit_zmumu_1.eps");
  return 0;
}
