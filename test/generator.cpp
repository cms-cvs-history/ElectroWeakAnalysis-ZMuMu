...
double yield(50550),Tk(.9883),Sa(.9896),Hlt(.9155),Iso(.9786);
int expt(1),seed(1);
...
MuTag mu(double Tk, double Sa) const {
  if( eventGenerator->Rndm()< Tk && eventGenerator->Rndm()< Sa ){
    return globalMu;
  } else if(eventGenerator->Rndm()< Tk){
    return trackerMu;
  }
  else if(eventGenerator->Rndm()< Sa){
    return standaloneMu;
  }
...
int main(int argc, char * argv[]){
  ...
  while ((o = getopt(argc, argv,"p:n:s:y:T:S:H:I:h"))!=EOF) {
    ...
    case 'y':
      ...
      break;
    case 'T':
      Tk  = strtod(optarg,&endPtr);
      break;
    case 'S':
      Sa = strtod(optarg,&endPtr);
      break;
    case 'H':
      Hlt = strtod(optarg,&endPtr);
      break;
    case 'I':
      Iso  = strtod(optarg,&endPtr);
      break;
    case 'h':
  ...
  for(int j = 0; j <expt; ++j){ 
	...
	if(rISO1< Iso && rISO2 < Iso){//two global mu isolated
	  if(rHLT1< Hlt && rHLT2 < Hlt) N2HLT++;
	  else if((rHLT1< Hlt && !rHLT2 < Hlt)||(!rHLT1 < Hlt && rHLT2 < Hlt)) N1HLT++;
	}else if(!rISO1< Iso || !rISO2 < Iso){//at least one not iso
	  if( rHLT1 < Hlt || rHLT2 < Hlt) NISO++;
	}
      }else if((mu1 == globalMu && mu2 == trackerMu && rHLT1< Hlt ) || (mu2 == globalMu && mu1 == trackerMu && rHLT2< Hlt)){
	if(rISO1< Iso && rISO2 < Iso) NTk++;
      }else if((mu1 == globalMu && mu2 == standaloneMu && rHLT1< Hlt) ||(mu2 == globalMu && mu1 == standaloneMu && rHLT2< Hlt)){
	if(rISO1< Iso && rISO2 < Iso) NSa++;
      }
